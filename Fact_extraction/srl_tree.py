...
def get_batch_trees(srl_parse_res):
	return [one_summary(item) for item in srl_parse_res]
if __name__ == '__main__':
	input_filename = "./XSum.srl." + sys.argv[1]
	output_filename = "./XSum.tree." + sys.argv[1]
	fpout = open(output_filename, "w")
	for line in open(input_filename):
		srl_res = json.loads(line.strip())
		if "filename" not in srl_res or \
			"summary" not in srl_res or \
			"document" not in srl_res or \
			"srl_summary" not in srl_res or \
			"srl_document" not in srl_res:
			continue
		summary_tree = one_summary(srl_res["srl_summary"])
		document_trees = get_batch_trees(srl_res["srl_document"])
		out_json = {}
		out_json["filename"] = srl_res["filename"]
		out_json["summary"] = srl_res["summary"]
