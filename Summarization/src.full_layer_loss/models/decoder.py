...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff, dropout, embeddings, loss_ws, layer_loss):
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None,
        ...
        if state.cache is None:
            state = state.update_state(tgt, saved_inputs, saved_attn, self.loss_ws)
...
class TransformerDecoderState(DecoderState):
    ...
    def detach(self):
        ...
        if self.loss_ws is not None:
            self.loss_ws = self.loss_ws.detach()
        self.src = self.src.detach()

    def update_state(self, new_input, previous_layer_inputs, saved_attn, loss_ws):
        state = TransformerDecoderState(self.src)
        state.previous_input = new_input
        state.previous_layer_inputs = previous_layer_inputs
        state.s2t_attn = saved_attn
        state.loss_ws = loss_ws
        return state
