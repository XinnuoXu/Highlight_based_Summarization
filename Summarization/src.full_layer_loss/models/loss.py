...
class NMTLossCompute(LossComputeBase):
    ...
    def _mean_with_mask(self, x, mask, m_dim=0):
        return x.sum(dim=m_dim) / mask.float().sum(dim=m_dim)

    def _normalization(self, x, mask):
        mean_x = self._mean_with_mask(x, mask, m_dim=1)
        xm = x.sub(mean_x[:, None])
        return xm.masked_fill(1-mask, 0.0)

    def _compute_loss(self, batch, output, target, src_mem_bank, alignment, mask_alg, mask_tgt, saved_attn, loss_ws):

        batch_size, heads_num, tgt_len, src_len = saved_attn.size()

        # CosineEmbeddingLoss
        alignment = alignment[:, :-1, :]
        #heads, batch_size, tgt_len, src_len = alignment.size()
        alignment = alignment.repeat(heads_num, 1, 1, 1)

        mask_alg = mask_alg[:, :-1, :]
        #heads, batch_size, tgt_len, src_len = mask_alg.size()
        mask_alg = mask_alg.repeat(heads_num, 1, 1, 1)

        saved_attn = saved_attn.transpose(0, 1)
        attn = saved_attn.masked_fill(1-mask_alg, -1e18)
        attn = self.softmax(attn)

        alignment = alignment.contiguous().view(-1, src_len)
        mask_alg = mask_alg.contiguous().view(-1, src_len)
        attn = attn.view(-1, src_len)
        y = torch.ones(attn.size()[0]).cuda()
        attn_loss = self.criterion_attn(self._normalization(alignment, mask_alg), self._normalization(attn, mask_alg), y)

        attn_loss = attn_loss.view(heads_num, batch_size, tgt_len)
        mask_tgt = mask_tgt[:, :-1]
