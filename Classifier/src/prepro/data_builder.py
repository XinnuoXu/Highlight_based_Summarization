...
class BertData():
    ...
    def preprocess(self, src, tgt, n_tgt, use_bert_basic_tokenizer=False, is_test=False):
        ...
        tgt = tgt[0]
        tgt_subtokens_str = self.cls_token + ' ' + ' '.join(self.tokenizer.tokenize(' '.join(tgt), use_bert_basic_tokenizer=use_bert_basic_tokenizer)) + ' ' + self.sep_token
        ...
        n_tgt = n_tgt[0]
        n_tgt_subtokens_str = self.cls_token + ' ' + ' '.join(self.tokenizer.tokenize(' '.join(n_tgt), use_bert_basic_tokenizer=use_bert_basic_tokenizer)) + ' ' + self.sep_token
        n_tgt_subtoken = n_tgt_subtokens_str.split()[:self.args.max_tgt_ntokens]
        if ((not is_test) and len(n_tgt_subtoken) < self.args.min_tgt_ntokens):
            return None
        n_tgt_subtoken_idxs = self.tokenizer.convert_tokens_to_ids(n_tgt_subtoken)

        # cut src
        tgt_max_len = max(len(tgt_subtoken_idxs), len(n_tgt_subtoken_idxs))
        src_chunk_len = min(self.args.max_pos - tgt_max_len, len(src_subtoken_idxs))
        src_subtoken_idxs = src_subtoken_idxs[:src_chunk_len]

        tmp_id = len(src_subtoken_idxs)-1
        while tmp_id > -1:
            if src_subtoken_idxs[tmp_id] == self.sep_vid:
                break
            tmp_id -= 1
        src_subtoken_idxs = src_subtoken_idxs[:tmp_id+1]
        segments_ids = segments_ids[:len(src_subtoken_idxs)]

        p_pair = src_subtoken_idxs + tgt_subtoken_idxs
        n_pair = src_subtoken_idxs + n_tgt_subtoken_idxs

        seg_id = 1 - segments_ids[-1]
        p_segments_ids = segments_ids + [seg_id] * len(tgt_subtoken_idxs)
        n_segments_ids = segments_ids + [seg_id] * len(n_tgt_subtoken_idxs)

        p_summ_mask = [0] * len(src_subtoken_idxs) + [1] * len(tgt_subtoken_idxs)
        n_summ_mask = [0] * len(src_subtoken_idxs) + [1] * len(n_tgt_subtoken_idxs)

        tgt_txt = ' '.join(tgt)
        n_tgt_text = ' '.join(n_tgt)
        src_txt = [original_src_txt[i] for i in idxs]

        return p_pair, n_pair, p_segments_ids, n_segments_ids, p_summ_mask, n_summ_mask, src_txt, tgt_txt, n_tgt_text
...
def _format_to_bert(params):
    ...
    for d in jobs:
        ...
        b_data = bert.preprocess(source, tgt, n_tgt, use_bert_basic_tokenizer=args.use_bert_basic_tokenizer, is_test=is_test)
        if (b_data is None):
            continue
        p_pair, n_pair, p_segments_ids, n_segments_ids, p_summ_mask, n_summ_mask, src_txt, tgt_txt, n_tgt_text = b_data
        b_data_dict = {"pos": p_pair, "neg": n_pair,
                       "p_summ_mask": p_summ_mask, 'n_summ_mask': n_summ_mask,
                       "p_segs": p_segments_ids, "n_segs": n_segments_ids,
                       'src_txt': src_txt, "tgt_txt": tgt_txt, "n_tgt_text": n_tgt_text}
...
def format_xsum_to_lines_easy(args):
    ...
    for corpus_type in datasets:
        ...
        for i, src in enumerate(srcs):
            json_objs.append({'src': src, 'tgt': [tgts[i][0].split()], 'n_tgt':[tgts[i][1].split()]})
